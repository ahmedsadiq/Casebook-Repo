-- =============================================================
-- Ensure all associate RLS policies are in their correct final
-- state regardless of which version of migration 003 was applied.
--
-- Migration 004 could partially break things if the current
-- (non-breaking) migration 003 had already been applied, because
-- 004 dropped "case_updates: associate insert" before failing on
-- the duplicate "case_updates: associate insert and view" create.
--
-- This migration is fully idempotent: it drops then recreates
-- every associate policy. It does NOT touch any row data.
-- =============================================================
...
-- Also drop any name variants left from the original broken migration 003
